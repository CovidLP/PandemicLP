...
#' \eqn{a\le b^f 0.08 Pop}, where \eqn{Pop} is the location's population.
...
#' over-reporting if \eqn{d > 1}.}
#' \subsection{Multiple curves}{
#' Additionally, two curves can be fitted, as happened in the Covid-19 pandemic
#' in many locations. In this case the model is slightly different. In this
#' case,
...
#' asymmetry in the curve, similarly to parameter \eqn{f}.}
#' @section Prior distribution:
#' Apart from the truncation mentioned above, the prior is defined as below.
#' Note that every available model used in the \code{\link{pandemic_model}}
#' function uses only a subset of these parameters. The priors are described
#' here for reference.
...
#'   fitted. By changing it to 2 implies a two waves model. Future versions of
#'   the package will allow for more waves.}
#' Seasonal and multiple waves cannot currently be used in the same model at
#' the same time. Future versions of the package will allow such mixtures and
#' also other distributions for the data.
#' @seealso \code{\link{pandemic_model}} and
#' \code{\link{posterior_predict.pandemicEstimated}}.
#' @references
#' CovidLP Team, 2020. CovidLP: Short and Long-term Prediction for COVID-19.
#' Departamento de Estatistica. UFMG, Brazil. URL:
